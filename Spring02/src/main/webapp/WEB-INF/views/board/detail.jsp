...
<html>
	...
	<body>		
		<div class="container-fliud">
			...
			<nav>	<%-- 페이지 이동 메뉴 --%>
				<ul>
					...
					<%-- 로그인 사용자 아이디와 글 작성자 아이디가 일치할 때만 수정메뉴를 보여줌 --%>
		...
		$(document).ready(function() {
			...
			var boardNo = $('#bno').val();	// 아이디 값 읽기
			
			// 게시글 번호(boardNo)에 달려 있는 모든 댓글 목록을 읽어오는 Ajax 함수
			...
			// $.getJSON(요청 URL, 콜백 함수): URL로 Ajax GET 요청을 보내고 JSON 문자열을 응답으로 전달받아서 처리
				$.getJSON('/ex02/replies/all/' + boardNo, function(respText) {
					...
					$('#replies').empty();	// div[id='replies']의 모든 하위 요소들을 삭제 (비워줌)
					
					var list = '';	// div[id='replies']의 하위 요소(HTML코드)를 작성할 문자열
					
					// 배열 respText의 원소들을 하나식 꺼내서 콜백 함수를 호출
					$(respText).each(function() {	// each는 for(:)와 비슷
						var date = new Date(this.regdate);	// JavaScript의 Date 객체 생성
						var dateStr = date.toLocaleDateString() + ' ' + date.toLocaleTimeString();	
						// 날짜 객체를 문자열로 변환하는 메소드
						list += '<div class="reply_item">'
								+ '<input type="text" id="rno" name="rno" value="'
								+ this.rno + '" readonly />'	// 열고 닫는 따옴표 주의
								+ '<input type="text" id="rtext" name="rtext" value="'
								+ this.rtext + '" />'
								+ '<input type="text" id="userid" name="userid" value="'
								+ this.userid + '" readonly />'
			...
			// 수정, 삭제 버튼에 대한 이벤트 리스너는 버튼들이 만들어진 이 후에 등록되어야 함
			$('#replies').on('click', '.reply_item .reply_update', function() {
				// 수정 버튼이 포함된 div 요소에 포함된 rno와 rtext를 찾아서 Ajax 요청을 보냄
