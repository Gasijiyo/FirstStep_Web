...
// signin POST 방식 컨트롤러 메소드 리턴 후 .
// 로그인 성공이면 가고자 했었던 url로 이동, 
// 로그인 실패면 다시 signin GET 요청으로 redirect
public class SignedInInterceptor implements HandlerInterceptor {
	...
	public void postHandle(HttpServletRequest request, HttpServletResponse response, Object handler,
		 ...
		 if(request.getMethod().equals("GET")) {
			 return;	// GET방식에서는 interceptor가 할 일이 없음. (로그인 페이지 form을 보여줌)
		 }
		 ...
		 if(user != null) {	//테이블에 아이디/비밀번호가 일치하는 사용자가 있는 경우 -> 로그인 성공
			 ...
			 // 원래 이동하려 했던 페이지(targetURL)로 redirect
