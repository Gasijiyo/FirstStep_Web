...
public class UserController {
	...
	public String register(User user) {	// ctrl+shift+o 자동 import
	...
	@ResponseBody
	// controller 메소드가 리턴하는 문자열이 jsp파일 경로이거나 혹은 페이지 이동(forward, redirect)주소가 아니면
	// 클라이언트로 전송되는 응답(response)문자열임을 선언하는 어노테이션
	public String checkUserId(String userid) {
		log.info("checkUserId({}) 호출", userid);
	...
	public void signIn(String url, Model model) {
		...
		// 로그인 페이지가 요청됐을 때, 로그인 성공 후 이동할 페이지가 질의문자열에 포함된 경우
		if (url != null && !url.equals("")) { 
			model.addAttribute("url",url);	// 로그인 이 후 이동할 페이지를 저장.
		}
	...
	public void signIn(User user, Model model) {
		...
		log.info("signInUser: {}", signInUser);	// 로그인 okay: not null, 로그인x : null
		
		// 로그인 여부를 판단할 수 있는 정보를 Model객체에 속성으로 저장.
	...
	public String signOut(HttpSession session) {
		log.info("signOut() 호출");
		// 세션에 저장된 로그인 정보(로그인 사용자 아이디)를 제거
		// -> main 으로 이동
