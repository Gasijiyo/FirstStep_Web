...
// RestController 클래스의 모든 메소드들이 리턴하는 값은 view(JSP)를 찾기 위한 문자열이 아니라
...
public class ReplyRestController {
	...
	public ResponseEntity<List<Reply>> readAllReplies(@PathVariable(name = "bno") Integer bno)	{
		...
		// Service 객체의 메소드를 호출해서 해당 글 번호(bno)의 모든 댓글 리스트를 작성.
		List<Reply> replyList = replyService.select(bno);
		
		// 댓글 리스트를 ResponseEntity 객체에 넣어서 클라이언트로 전송.
	...
	public ResponseEntity<Integer> createReply(@RequestBody Reply reply) {
		...
		// 요청 시 보내는 데이터가 query string에 포함되어 있지 않고, form 데이터도 아닌 경우에 사용   
	...
	public ResponseEntity<Integer> updateReply(
		...
		reply.setRno(rno);	// PathVariable 값으로 Reply 인스턴스의 rno 값(수정할 댓글 번호) 설정
