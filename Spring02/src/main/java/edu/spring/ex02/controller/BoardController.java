...
@Controller	// Dispatcher Servlet에 의해서 컴포넌트 객체로 생성되고 관리됨.
...
public class BoardController {
	...
	public void main(Model model) {
		log.info("main() 호출");
		// controller 메소드의 리턴 문자열이 없으면 요청주소로 View(jsp파일)을 찾음.
		
		// Service 객체의 메소드를 사용해서 글 전체 목록을 가져옴.
		List<Board> list = boardService.select();
		// 글 전체 목록을 Model 객체에 속성으로 추가해서 View(jsp)에게 전달
		// Model 파라미터 입력  
	...
	public String insert(Board board)	{
		...
		// 클라이언트에서 보낸 데이터들을 서비스 계층의 객체(메소드)를 사용해서
		// 새 글 작성 서비스 완료 후 게시판메인으로 이동(redirect)
	...
	public void detail(int bno, Model model) {
		...
		// TODO: 서비스 계층의 객체(메소드)를 사용해서 해당 글 번호(bno)를 검색하고
		// 검색 된 내용을 View(jsp)로 전달
